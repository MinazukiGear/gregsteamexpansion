...
- **蒸汽粉碎机 / 大型蒸汽粉碎机 / 电力粉碎机**：独立于研磨机的第一段矿物处理多方块（蒸汽双机）与 LV–UV 分级单方块电力机器，共享矿石粉碎迁移配方（矿石主产物 4× 基线 × 难度倍率），支持 EMI 结构预览。
...
- **轻量蒸汽多方块家族**：蒸汽压缩机（C1a）、蒸汽提取机（C1b）、蒸汽锻压机（B6）——3×3×3 级紧凑结构、并行 8、LV 电压门、无排气仓。
...
- **锻铁大宗配方注入**（随 G1 落地）：向 `primitive_blast_furnace` 类型 add-only 注入 6 条"铁粉 + 燃料 → 锻铁"配方，原始高炉同步可用，补足上游仅有铁粒零散路线（铁粒烧锻铁粒为上游既有 `gtceu:wrought_iron_nugget`，本模组不重复添加）的锻铁产能缺口。
...
各机器的实际游戏测试与最终验收由使用者执行；对应验收清单位于各设计文档末尾。

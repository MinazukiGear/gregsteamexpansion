...
.\gradlew.bat runGameTestServer     # 运行全部 GameTest（120 个，见下）
...
| `GSESteamEngineTests` | 四类引擎状态边界、原子取汽、并行/输出容量、多产物槽位竞争、物品与流体批次结算、排气和鼓风、熔炉独立输入仓隔离与轮询、控制器 NBT 往返及真实区块落盘重载、大型蒸汽供给仓超频批次锁定、大型蒸汽高炉锻铁—炼钢生产闭环、96 并行满载供汽/鼓风边界、排气受阻与缺汽分界、三重并行限制、偏好配方重载与八进气室持续采集，以及创造/ME/样板总成接口兼容（40 个） |

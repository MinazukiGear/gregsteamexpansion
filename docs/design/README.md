...
3. 控制器核心运行状态与待输出的 NBT 往返、处理机真实区块落盘重载和完整服务器进程重启、完整流体加工链及多产物竞争剩余容量已接入自动化；继续补充配置重启等覆盖。自动化结果与手动验收分别记录。

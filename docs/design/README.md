...
3. 控制器核心运行状态与待输出的 NBT 往返已接入自动化；继续补充真实区块卸载/世界重载、完整流体加工链、多产物竞争剩余容量等覆盖。自动化结果与手动验收分别记录。

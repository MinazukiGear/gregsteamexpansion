...
状态核对日期：2026-09-12；代码基线：`fda7ea4`。当前处于 Alpha，主任务是验证生存获取与生产闭环，并完成游戏内验收。工程重构 P0～P2 已完成，历史实现和门禁记录见[工程路线图](../refactor-roadmap.md)。

“已定案”表示设计规则已确认；“已实现”表示已有代码与资源，不保证每条规格均已验证；“已验收”只记录用户实际执行并确认的结果。本次文档同步不增加验收通过项，也不将代码与定案的差异自动改写为新规则。
...
2. 按机器规格核验断汽恢复、输出堵塞、结构拆建、存档重载、难度切换，以及 GUI/Jade、EMI 和外部容器交互。
3. 补充存档/NBT 往返、完整流体加工链、多产物竞争剩余容量等自动化覆盖；自动化结果与手动验收分别记录。
4. 核对以下静态发现的设计/实现差异，按原定案要求修复；若需改变玩法规则，则独立讨论。

| 待核对项 | 已有证据与处理边界 |
| --- | --- |
| 锅炉房进气室的位置与数量 | `GSEBoilerPatterns` 在顶面和第二层内部设置 `A` 候选位，允许外壳或进气室；图案没有精确数量限制。与“仅 1 个、顶面中心”的设计不同，详见[锅炉房](boiler-room.md)。 |
...
按 Markdown `- [ ]` 统计，当前 16 份规格共有 204 项未勾选验收项。焦炉、合成站、熔炉等还使用表格、编号或普通列表记录核验范围，因此 204 不是全项目待办总数。

## 候选与验证边界

[后续机器候选清单](next-machine-candidates.md)中的 C4 发酵厂、C5a 筛分厂、C5b 磁选厂及其他新方向均未立项；先核验配方生态与闭环需求，再决定是否扩展。

全仓库现有 65 个 GameTest。2026-09-12 的历史复验记录见[工程路线图](../refactor-roadmap.md#完成态基线2026-09-12)，当前验证命令见[项目 README](../../README.md#测试与-ci)。历史通过记录不代表每次文档更新都重新运行了构建，也不替代用户验收。

...
生存链进度（2026-09-13）：第一批 Expert 基础材料工作台路线已验收。工业蒸汽机械方块与青铜构件均可在进入电力时代前按规格合成，工具只损耗耐久，EMI 只显示当前启动档位的配方；Easy、Normal 与组装机路线仍待后续批次验证。第二批蒸汽供给链已验收：新供给仓可正常合成，只接受并缓存最多 `32,000 mB` 标准蒸汽，能把锅炉蒸汽交给多方块执行配方；旧仓从正常入口隐藏且旧仓物品放置时转换。旧存档方块实体与超额存量迁移仍待单独验证。第三批 Expert 蒸汽粉碎链已验收：蒸汽碾磨方块与控制器可正常获取，`3×3×3` 结构成型并处理矿石方块和粗矿，单并行按 `600 tick`、`200 mB/t` 运行，产物倍率与 EMI 迁移去重符合规格。第四批锻铁配方门控已验收：三条纯粉锻铁路线正确显示，上游原始高炉拒绝执行它们，原有炼钢、熔铁和铁粒锻铁路线不受影响。第五批基础运行冒烟测试由用户确认：全部机器均可在游戏内正常运行；该结果覆盖基本可运行性，不代替尚未逐项记录的获取路线、存档迁移、堵塞恢复、跨模组接口和数值边界验收。

...
生存链进度（2026-09-13）：第一批 Expert 基础材料工作台路线已验收。工业蒸汽机械方块与青铜构件均可在进入电力时代前按规格合成，工具只损耗耐久，EMI 只显示当前启动档位的配方；Easy、Normal 与组装机路线仍待后续批次验证。第二批蒸汽供给链已验收：新供给仓可正常合成，只接受并缓存最多 `32,000 mB` 标准蒸汽，能把锅炉蒸汽交给多方块执行配方；旧仓从正常入口隐藏且旧仓物品放置时转换。旧存档方块实体与超额存量迁移仍待单独验证。

...
3. 控制器核心运行状态与待输出的 NBT 往返、处理机完整流体加工链已接入自动化；继续补充真实区块卸载/世界重载、多产物竞争剩余容量等覆盖。自动化结果与手动验收分别记录。
...
全仓库现有 89 个 GameTest（锅炉房共 11 个，启动难度预设 1 个）。其中 4 项控制器持久化测试覆盖处理机、粉碎机、蓄热熔炉和虚空生产机的方块实体 NBT 往返，并验证恢复后的待输出缓冲仍能无丢失、无重复地交付物品与流体；真实区块卸载和世界重载仍需单独验证。另有 1 项处理机完整流体加工测试覆盖物品与流体输入扣取、并行锁定、蒸汽总耗和物品/流体产物结算，防止配方完成时漏掉流体产物。难度测试逐项核对当前启动档位写入 GTCEu 的 19 个配方难度布尔项与 `casingsPerCraft`。其中 6 项接口矩阵测试覆盖创造输入、GTCEu ME 输入、ME 样板总成、GTCEu ME 输出与 GTM Things ME 输出缓冲的结构成型和运行时收集。锅炉房完整燃料启动链覆盖：标准流体仓注入水与杂酚油、原生或 GTM Things 创造物品输入总线注入煤粉、进气室供气后，配方必须开始推进并升温；同时锁定背面消音器朝外且出口无阻挡。2026-09-12 的 65 项历史复验记录见[工程路线图](../refactor-roadmap.md#完成态基线2026-09-12)，当前验证命令见[项目 README](../../README.md#测试与-ci)。历史通过记录不代表每次文档更新都重新运行了构建，也不替代用户验收。

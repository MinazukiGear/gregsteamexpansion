...
状态核对日期：2026-09-13；发布基线：`v0.1.0-alpha.4`。当前处于 Alpha 测试阶段；全部机器的基础游戏内运行冒烟测试已由用户确认通过，主任务转为验证生存获取与生产闭环、存档稳定性、异常恢复和跨模组自动化。工程重构 P0～P2 已完成，历史实现和门禁记录见[工程路线图](../refactor-roadmap.md)。
...
3. 控制器核心运行状态与待输出的 NBT 往返、处理机真实区块落盘重载、完整流体加工链及多产物竞争剩余容量已接入自动化；继续补充完整服务器重启、配置重启等覆盖。自动化结果与手动验收分别记录。

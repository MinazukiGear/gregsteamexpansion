...
3. 控制器核心运行状态与待输出的 NBT 往返、处理机完整流体加工链及多产物竞争剩余容量已接入自动化；继续补充真实区块卸载/世界重载、配置重启等覆盖。自动化结果与手动验收分别记录。

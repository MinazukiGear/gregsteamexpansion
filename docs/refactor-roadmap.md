...
**后续排期**：P2-7 → P0-1 → P1-3 / P1-4 → P2-6 / P2-9 剩余项。
...
| P2-7 | 尚待系统补齐。当前共有 50 个 `@GameTest`；已有缓存、进气室及结构诊断测试不能替代引擎状态机安全网。下一步优先补并行/输出边界、缺汽回退、输出阻塞恢复和结构失效保留，再覆盖排气、鼓风与机会产出。 |
...
本轮验证：JDK 17 下 `compileJava runGameTestServer build` 成功，全部 50 个 GameTest 通过（扩展既有测试，未新增测试方法）。
...
| 顺序 | 项 | 理由 |
| --- | --- | --- |
| 1 | **P2-7 补引擎行为测试** | 下一步；P0-1 的前置安全网 |
| 2 | **P0-1 引擎基类抽取** | 按 5 个粒度分批提交，每步运行回归 |
| 3 | **P1-3 / P1-4 数据驱动化** | 引擎边界稳定后推进 |
| 4 | **P2-6 / P2-9 剩余项** | 可穿插处理，优先补 Jade 配置翻译完整性检查 |

P1-5、P0-2 与 P2-8 已有实现，后续继续由门禁保护，不再列为从零开工的任务。

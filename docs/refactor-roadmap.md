...
4. **补齐高风险自动化缺口**：四类蒸汽引擎的控制器 NBT 往返、处理机真实区块落盘重载与流体产物完整链路，以及处理机/粉碎机的多产物剩余容量竞争已由 8 项 GameTest 覆盖。`tools/verify_server_restart.py` 以两个独立 JVM 覆盖控制器完整服务器进程重启；`tools/verify_config_restart.py` 以 Easy 与 Expert 两套启动配置核对档位、GTCEu 配方预设、旗舰机器开关和权重表，并恢复原配置。后续优先考虑结构诊断边界。纯视觉与交互项仍保留为用户手动验收。

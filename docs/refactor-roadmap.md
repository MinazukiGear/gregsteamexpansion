...
4. **补齐高风险自动化缺口**：四类蒸汽引擎的控制器 NBT 往返、处理机真实区块落盘重载与流体产物完整链路，以及处理机/粉碎机的多产物剩余容量竞争已由 8 项 GameTest 覆盖。真实区块测试另由 `tools/verify_server_restart.py` 分两个独立 JVM 执行写入与核验阶段，覆盖完整服务器进程重启；后续优先考虑配置重启和结构诊断边界。纯视觉与交互项仍保留为用户手动验收。
